...
const DB_PATH = path.join(__dirname, 'hunt.db');
...
async function startServer() {
  ...
  // Safe column adds
  ['ALTER TABLE claims ADD COLUMN answer_text TEXT',
   'ALTER TABLE claims ADD COLUMN answer_correct INTEGER DEFAULT 0',
   'ALTER TABLE messages ADD COLUMN image_url TEXT',
   'ALTER TABLE messages ADD COLUMN player_id TEXT',
   'ALTER TABLE messages ADD COLUMN is_system INTEGER DEFAULT 0',
   'ALTER TABLE messages ADD COLUMN team_id TEXT',
   'ALTER TABLE messages ADD COLUMN team_name TEXT',
   'ALTER TABLE messages ADD COLUMN team_color TEXT',
   'ALTER TABLE players ADD COLUMN ready INTEGER DEFAULT 0',
  ].forEach(sql => { try { db.run(sql); } catch {} });

  saveDb();

  function getConfig(key, def = null) {
    const row = prepare('SELECT value FROM hunt_config WHERE key = ?').get(key);
    return row ? row.value : def;
  }
  function setConfig(key, value) {
    prepare('INSERT OR REPLACE INTO hunt_config (key, value) VALUES (?, ?)').run(key, String(value));
  }
  ...
  // ── Helpers ─────────────────────────────────────────────────────────────────
  function getState() {
    const teams = prepare('SELECT * FROM teams ORDER BY created_at ASC').all();
    const players = prepare('SELECT * FROM players ORDER BY created_at ASC').all();
    const claims = prepare('SELECT * FROM claims ORDER BY claimed_at ASC').all();
    const messages = prepare('SELECT * FROM messages ORDER BY sent_at ASC LIMIT 400').all();
  ...
  function postSystemMessage(text) {
    const id = uuidv4();
    prepare('INSERT INTO messages (id, sender_name, text, is_system) VALUES (?, ?, ?, 1)').run(id, 'system', text);
    const msg = prepare('SELECT * FROM messages WHERE id = ?').get(id);
  ...
  function scheduleCountdownMessages(startTime, totalMinutes) {
    ...
    function schedule(atMs, fn) { const delay = atMs - elapsed; if (delay > 0) countdownTimers.push(setTimeout(fn, delay)); }
    schedule(totalMs - 30*60*1000, () => { postSystemMessage('⏰ 30 minutes remaining in the hunt!'); broadcastState(); });
    schedule(totalMs - 10*60*1000, () => { postSystemMessage('🚨 Only 10 minutes left — finish your challenges!'); broadcastState(); });
    schedule(totalMs, () => { postSystemMessage('🏁 Time is up! Return to the pub and await the final results…'); broadcastState(); });
  }
  ...
  app.post('/api/players/join', (req, res) => {
    ...
    if (team_id) { if (!prepare('SELECT id FROM teams WHERE id = ?').get(team_id)) return res.status(400).json({ error: 'Team not found' }); }
    let player = prepare('SELECT * FROM players WHERE name = ?').get(name.trim());
    if (player) {
      if (team_id && team_id !== player.team_id) prepare('UPDATE players SET team_id = ? WHERE id = ?').run(team_id, player.id);
      player = prepare('SELECT * FROM players WHERE id = ?').get(player.id);
    } else {
      const id = uuidv4();
      try { prepare('INSERT INTO players (id, name, team_id) VALUES (?, ?, ?)').run(id, name.trim(), team_id || null); player = prepare('SELECT * FROM players WHERE id = ?').get(id); }
  ...
  app.post('/api/players/:id/ready', (req, res) => {
    prepare('UPDATE players SET ready = ? WHERE id = ?').run(req.body.ready ? 1 : 0, req.params.id);
  ...
  app.post('/api/teams', (req, res) => {
    ...
    const existing = prepare('SELECT * FROM teams').all();
    ...
    const id = uuidv4();
    try { prepare('INSERT INTO teams (id, name, color) VALUES (?, ?, ?)').run(id, name.trim(), color); }
  ...
  app.post('/api/claim', upload.single('media'), (req, res) => {
    ...
    const existing = prepare('SELECT c.team_id, t.name as tname FROM claims c JOIN teams t ON c.team_id=t.id WHERE c.challenge_id=? AND c.is_bonus=?').get(challenge_id, isBonus);
    ...
    try { prepare('INSERT INTO claims (id,team_id,challenge_id,is_bonus,photo_url,answer_text,answer_correct) VALUES (?,?,?,?,?,?,?)').run(uuidv4(), team_id, challenge_id, isBonus, media_url, answer_text||null, answer_correct); }
    catch(e) { if (e.message?.includes('UNIQUE')) return res.status(409).json({ error: 'Your team already claimed this' }); throw e; }
    const team = prepare('SELECT * FROM teams WHERE id=?').get(team_id);
    if (team && ch) {
    ...
    broadcastState(); res.json({ success: true, media_url, answer_correct });
  });
  ...
  app.post('/api/unclaim', (req, res) => {
    ...
    const claim = prepare('SELECT * FROM claims WHERE team_id=? AND challenge_id=? AND is_bonus=?').get(team_id, challenge_id, isBonus);
    if (!claim) return res.status(404).json({ error: 'Claim not found' });
    if (claim.photo_url) { try { fs.unlinkSync(path.join(UPLOADS_DIR, claim.photo_url.replace('/uploads/',''))); } catch {} }
    prepare('DELETE FROM claims WHERE team_id=? AND challenge_id=? AND is_bonus=?').run(team_id, challenge_id, isBonus);
    const team = prepare('SELECT * FROM teams WHERE id=?').get(team_id);
    const baseId = challenge_id.split('_bonus_')[0];
    const ch = CHALLENGES.find(c => String(c.id) === baseId);
    if (team && ch) postSystemMessage(`↩ ${team.name} unclaimed "${ch.title}"${is_bonus?' (bonus)':''} — it's up for grabs again!`);
    broadcastState(); res.json({ success: true });
  });
  ...
  app.post('/api/messages', chatUpload.single('image'), (req, res) => {
    ...
    prepare('INSERT INTO messages (id,player_id,team_id,team_name,team_color,sender_name,text,image_url) VALUES (?,?,?,?,?,?,?,?)').run(id, player_id||null, team_id, team_name, team_color, sender_name, msgText, image_url);
    const msg = prepare('SELECT * FROM messages WHERE id = ?').get(id);
    broadcast('new_message', msg); res.json(msg);
  });

  app.get('/api/download/photos', (req, res) => {
    const state = getState();
    const allClaims = prepare('SELECT * FROM claims WHERE photo_url IS NOT NULL').all();
    const chatMsgs = prepare('SELECT * FROM messages WHERE image_url IS NOT NULL').all();
    const teamMap = {};
    ...
    allClaims.forEach(c => {
      ...
      arc.file(fp, { name: `challenges/${tn}/challenge-${c.challenge_id.replace('_bonus_','-bonus-')}${c.is_bonus?'-BONUS':''}${path.extname(fn)}` });
    });
    chatMsgs.forEach(m => {
    ...
    arc.append(chatLog, { name: 'chat/chat-log.txt' });
...
<\/script></body></html>`;
  ...
  app.post('/api/admin/hunt', (req, res) => {
    const { password, action, countdown_minutes, adjust_minutes } = req.body;
    ...
    if (action === 'start') {
      ...
      prepare('UPDATE players SET ready=0').run();
      ...
      if (adjust_minutes) {
        ...
        const sign = delta > 0 ? '+' : '';
        postSystemMessage(`⏱ Hunt duration adjusted by ${sign}${delta} minutes. ~${remaining} minutes remaining.`);
        if (startTime) scheduleCountdownMessages(startTime, newMins);
      }
  ...
  app.post('/api/admin/force-claim', (req, res) => {
    ...
    if (password !== ADMIN_PASSWORD) return res.status(403).json({ error: 'Wrong password' });
    const isBonus = is_bonus ? 1 : 0;
    prepare('DELETE FROM claims WHERE challenge_id=? AND is_bonus=?').run(challenge_id, isBonus);
    prepare('INSERT INTO claims (id,team_id,challenge_id,is_bonus,answer_correct) VALUES (?,?,?,?,1)').run(uuidv4(), team_id, challenge_id, isBonus);
    broadcastState(); res.json({ success: true });
  });
  ...
  app.post('/api/admin/remove-claim', (req, res) => {
    ...
    if (password !== ADMIN_PASSWORD) return res.status(403).json({ error: 'Wrong password' });
    const claim = prepare('SELECT * FROM claims WHERE challenge_id=? AND is_bonus=?').get(challenge_id, is_bonus ? 1 : 0);
    if (claim?.photo_url) { try { fs.unlinkSync(path.join(UPLOADS_DIR, claim.photo_url.replace('/uploads/',''))); } catch {} }
    prepare('DELETE FROM claims WHERE challenge_id=? AND is_bonus=?').run(challenge_id, is_bonus ? 1 : 0);
    broadcastState(); res.json({ success: true });
  });
  ...
  app.post('/api/admin/player', (req, res) => {
    ...
    if (password !== ADMIN_PASSWORD) return res.status(403).json({ error: 'Wrong password' });
    if (action === 'remove') { prepare('DELETE FROM players WHERE id=?').run(player_id); }
    else {
      if (name) { try { prepare('UPDATE players SET name=? WHERE id=?').run(name.trim(), player_id); } catch(e) { return res.status(409).json({ error: 'Name taken' }); } }
      if (team_id !== undefined) prepare('UPDATE players SET team_id=? WHERE id=?').run(team_id||null, player_id);
    }
    broadcastState(); res.json({ success: true });
  });
  ...
  app.post('/api/admin/team', (req, res) => {
    ...
    if (password !== ADMIN_PASSWORD) return res.status(403).json({ error: 'Wrong password' });
    if (action === 'remove') { prepare('UPDATE players SET team_id=NULL WHERE team_id=?').run(team_id); prepare('DELETE FROM teams WHERE id=?').run(team_id); }
    else if (name) { try { prepare('UPDATE teams SET name=? WHERE id=?').run(name.trim(), team_id); } catch(e) { return res.status(409).json({ error: 'Name taken' }); } }
    broadcastState(); res.json({ success: true });
  });
  ...
  app.post('/api/reset', (req, res) => {
    ...
    clearCountdownTimers();
    db.run('DELETE FROM claims; DELETE FROM teams; DELETE FROM players; DELETE FROM messages; DELETE FROM location_history;');
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('location_update', (data) => {
      ...
      if (getConfig('hunt_status') === 'active') {
        prepare('INSERT INTO location_history (id,player_id,player_name,team_id,team_color,lat,lng) VALUES (?,?,?,?,?,?,?)').run(uuidv4(), userId, name, teamId||null, teamColor||null, lat, lng);
      }
    });
  ...
  // Re-schedule countdown on restart
  if (getConfig('hunt_status') === 'active') {
    const startTime = parseInt(getConfig('hunt_start_time','0'));
